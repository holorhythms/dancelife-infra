// Locals definitions
locals {
    # Resource names
    admin_dashboard_name = "dancelife-admin-dashboard-${var.environment_name}"
    app_service_name = "dancelife-app-service-${var.environment_name}"
    app_service_insights_name = "dancelife-app-service-${var.environment_name}-insights"
    front_door_profile_name = "dancelife-fd-${var.environment_name}"
    front_door_endpoint_name = "dancelife-app-service-${var.environment_name}"
    front_door_waf_policy_name = "dancelifeappservicewaf${var.environment_name}"
    postgres_server_name = "dancelife-postgres-server-${var.environment_name}"
    resource_group_name = "dancelife-rg-${var.environment_name}"
    storage_account_name = "dancelifestorage${var.environment_name}"
    web_portal_name = "dancelife-web-portal-${var.environment_name}"
}
